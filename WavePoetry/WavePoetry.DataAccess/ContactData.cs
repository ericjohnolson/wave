...
namespace WavePoetry.DataAccess
{
    public class ContactData
    {
        ...
        {
            var cs = dbContext.contacts.Where(s => s.contact_contact_category.Select(c => c.contact_category_id).Contains(353) && s.contact_contact_category.Select(c => c.contact_category_id).Contains(361));
            string ids = "";
        ...
        public void Delete(int id)
        {
            ...
            foreach (var title in model.contact_title)
                title.author = 1;
            foreach (var cat in dbContext.contact_to_category.Where(s => s.contact_id == id).ToList())
                dbContext.contact_to_category.Remove(cat);
            foreach (var ship in dbContext.shipments.Where(s => s.contact_id == id).ToList())
                dbContext.shipments.Remove(ship);
            foreach (var review in dbContext.reviews.Where(r => r.reviewedby == id).ToList())
                dbContext.reviews.Remove(review);

            dbContext.contacts.Remove(model);
